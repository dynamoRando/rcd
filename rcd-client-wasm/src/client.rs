...
impl RcdClient {
    ...
    pub async fn auth_for_token_x(&mut self, un: &str, pw: &str) -> Result<Token, String> {
        use crate::x::AuthRequest;
        use crate::{x::sql_client_client::SqlClientClient};
        ...
        let r = rcd.auth_for_token(request).await.unwrap().into_inner();
