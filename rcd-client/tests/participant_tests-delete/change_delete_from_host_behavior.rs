pub mod grpc {
    ...
    */
    ...
    #[test]
    fn test() {
        let test_name = "delta_delete_from_host_gprc";
...

        {
            ...
            thread::spawn(move || {
                let res = main_delete_should_fail(&db_name, &main_client_addr);
                tx.send(res).unwrap();
            })
            .join()
            .unwrap();
...
pub mod http {
    ...
    */
    ...
    #[test]
    fn test() {
        let test_name = "delta_delete_from_host_http";
        ...
        thread::spawn(move || {
            let res = main_delete_should_fail(&db_name_copy, ma2);
