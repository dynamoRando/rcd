pub mod grpc {
    ...
    */
    ...
    #[test]
    fn test() {
        let test_name = "delete_from_host_with_log_gprc";
...

        {
            ...
            thread::spawn(move || {
                let res = main_execute_coop_write_and_read(&main_client_addr, &db);
                tx.send(res).unwrap();
            })
            .join()
            .unwrap();
...

        {
            ...
            thread::spawn(move || {
                let res = main_delete_should_succeed(&db, &main_client_addr);
                tx.send(res).unwrap();
            })
            .join()
            .unwrap();

            let should_succeed = rx.try_recv().unwrap();
...

        {
            ...
            thread::spawn(move || {
                let res = get_data_logs_at_participant(&db, &participant_client_addr);
                tx.send(res).unwrap();
            })
            .join()
            .unwrap();
...
pub mod http {
    ...
    */
    ...
    #[test]
    fn test() {
        let test_name = "delete_from_host_with_log_http";
        ...
        thread::spawn(move || {
            let res = main_delete_should_succeed(&db_name_copy, ma2);
        ...
        thread::spawn(move || {
            let res = get_data_logs_at_participant(&db_name_copy_, pa3);
