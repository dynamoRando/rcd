pub mod grpc {
    ...
    */
    ...
    #[test]
    fn test() {
        let test_name = "delta_update_from_host_gprc";
...

        {
            ...
            thread::spawn(move || {
                let res = main_update_should_fail(&db, &main_client_addr);
                tx.send(res).unwrap();
            })
            .join()
            .unwrap();

            let should_fail = rx.try_recv().unwrap();
...
pub mod http {
    ...
    */
    ...
    #[test]
    fn test() {
        let test_name = "delta_update_from_host_http";
        ...
        thread::spawn(move || {
            let res = main_update_should_fail(&db_name_copy, ma2);
