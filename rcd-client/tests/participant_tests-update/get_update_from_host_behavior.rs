pub mod grpc {
    ...
    */
    ...
    #[test]
    fn test() {
        let test_name = "get_update_from_host_gprc";
...

        {
            ...
            thread::spawn(move || {
                let res =
                    participant_service_client(&participant_client_addr, &contract);
                tx.send(res).unwrap();
            })
            .join()
            .unwrap();
...

        {
            ...
            thread::spawn(move || {
                let res = participant_changes_update_behavior(&db, &participant_client_addr, new_update_behavior);
                tx.send(res).unwrap();
            })
            .join()
            .unwrap();
    ...
    async fn participant_changes_update_behavior(
        ...
        let response = client
            ...
            .await
            .unwrap();
        let response_value = UpdatesFromHostBehavior::from_u32(response.behavior);

        response_value == behavior
    }
}
...
pub mod http {
    ...
    */
    ...
    #[test]
    fn test() {
        let test_name = "get_update_from_host_http";
        ...
        let _pa3 = participant_addrs.clone();
        ...
        thread::spawn(move || {
            let res = participant_changes_update_behavior(&pdn, pa2, new_update_behavior);
    ...
    async fn participant_changes_update_behavior(
        ...
        let response = client
            ...
            .await
            .unwrap();
        let response_behavior = UpdatesFromHostBehavior::from_u32(response.behavior);
