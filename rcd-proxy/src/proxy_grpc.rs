...
impl SqlClient for ProxyClientGrpc {
    async fn is_online(
        ...
        let reply = TestReply {
            ...
            reply_echo_message: request.into_inner().request_echo_message.clone(),
    ...
    async fn get_active_contract(
        ...
        match auth_result {
            Ok(core) => {
                let response = core.get_active_contact(request).await;
                return Ok(Response::new(response));
            }
            Err(auth_result) => {
...
impl ProxyDbGrpc {
    ...
    /// otherwise, we will return an `AuthResult` with an error message
    #[allow(dead_code, unused_variables)]
    fn validate_auth_request(&self, auth: &Option<AuthRequest>) -> Result<RcdData, AuthResult> {
        if auth.is_some() {
            let auth = auth.as_ref().unwrap().clone();
            let un = auth.user_name.clone();
            if auth.id.is_some() {
                let id = auth.id.unwrap();
                let result_has_core = self.proxy.get_rcd_core_data_for_existing_host_grpc(
                    &id,
                    &self.proxy.settings.grpc_db_addr_port,
                    60,
                );
...
#[tonic::async_trait]
impl DataService for ProxyDbGrpc {
    async fn is_online(
        &self,
        request: Request<TestRequest>,
    ) -> Result<Response<TestReply>, Status> {
    ...
    async fn create_partial_database(
        ...
        match auth_result {
            ...
            Err(auth_result) => {
                let reply = CreateDatabaseResult {
                    authentication_result: Some(auth_result),
                    is_successful: false,
