...
pub fn setup_main_and_participant(config: CoreTestConfig) {
    ...
    let pc = Arc::new(config.participant_client.unwrap().clone());
    let db = Arc::new(config.test_db_name.clone());
    let contract = Arc::new(config.contract_desc.unwrap().clone());
    let pda = Arc::new(config.participant_db_addr.unwrap().clone());
