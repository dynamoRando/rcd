...
pub const SQL_GET_ASSOCIATED_EVENTS: &str =
    "SELECT event_id, event_type, event_date, user_id FROM associated_event;";
...
pub async fn get_events(
    ...
    if let Ok(authenticated) = is_authenticated_result {
        if authenticated {
            let has_any_events_result =
                has_any_rows("SELECT COUNT(*) cnt from event", settings).await;
